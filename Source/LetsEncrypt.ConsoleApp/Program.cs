...
namespace LetsEncrypt.ConsoleApp
{
  internal class Program
  {
	...
	private static async Task Run()
	{
	  ...
	  if (Settings.Environment == "Stage" || Settings.Environment == "Dev")
	  {
		acmeClient = new AcmeClient(ApiEnvironment.LetsEncryptV2Staging);
	  }
	  else
	  {
		acmeClient = new AcmeClient(ApiEnvironment.LetsEncryptV2);
	  }

	  // Load existing accout or create new Account
	  Account account;
	  account = await Account.LoadAsync(Settings.ContactEmail);
	  if (account == null)
	  {
